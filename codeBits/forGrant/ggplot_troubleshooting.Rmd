---
title: "troubleshooting slow ggplot"
author: "Janet Young\n"
date: "`r Sys.Date()`\n"
...
mess with aesthetics (here I didn't save the updated plot but you could).  `theme_classic()` applies a whole bunch of changes
```{r}
myplot + theme_classic()
```

you can also change individual plot elements one at a time.  google searches are your friend for this (also `?theme`)
```{r}
myplot + 
...
make fake big data tibble (actually, I don't need this because I sneakily found Grant's real data):
```{r}
...
make smaller version of the tibble, and modified counts where we fake any values >150 to be 151
```{r, make smaller version of data and replace high counts}
...
```{r, code for slow plot, simplified to have only one histogram for testing}
...
Idea1: Try rendering to a file rather than screen (width/height are in inches). See if that’s faster than on screen.  No - still slow - not running right now
```{r, render slow plot to pdf}
...
```{r, render slow plot to Rmd doc}
# essential_vs_nonessential_vs_intergenic
```

try solution from [here](https://stackoverflow.com/questions/56607124/plotting-histogram-of-a-big-matrix-in-ggplot2-is-20x-slower-than-base-hist)

```{r, define get_binned_counts function}
...
for some aesthetic testing you can comment out 2/3 of the geom_rects temporarily to speed things up
```{r, plot_v2}
...
Maybe binsize is too small on the histogram- could you manage with lower binsize? 

Or, before you plot, remove the outliers that you’re not plotting anyway, so that it calculates way fewer bin counts ?
...
In any case you wanted to consolidate the >150 counts into a single category – you could do that in a hacky way like this:  (and in Illustrator, you’d add the >150 label on the plot)
...
that could actually speed things up a lot, as it would mean that relevant_counts_modified has a lot fewer histogram bins to calculate
